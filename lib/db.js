...
const DB_ROOT = process.env.NODE_ENV === 'production' ? path.join('/tmp', 'db') : path.join(process.cwd(), 'db');
const SCHEMA_PATH = path.join(process.cwd(), 'db', 'schema.sql');
if (!fs.existsSync(DB_ROOT)) fs.mkdirSync(DB_ROOT, { recursive: true });
const DB_PATH = process.env.DATABASE_PATH || path.join(DB_ROOT, 'tree-of-hope.db');
